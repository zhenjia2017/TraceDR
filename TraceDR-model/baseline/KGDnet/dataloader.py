...
def get_ehr_data(device):
  ...
  data_train = dill.load(open('../drugrec/data_train.pkl', 'rb'))
  # eval_len = int(len(data[split_point:]) / 2)
  data_test = dill.load(open('../drugrec/data_test.pkl', 'rb'))
  data_eval = dill.load(open('../drugrec/data_eval.pkl', 'rb'))
  ehr_records = data_train + data_test + data_eval

  ## Adjacency Matrices
  with open('../drugrec/diag_adj.pkl', 'rb') as f:
    diag_adj = pickle.load(f)

  with open('../drugrec/proc_adj.pkl', 'rb') as f:
    proc_adj = pickle.load(f)

  with open('../drugrec/diag_proc_adj.pkl', 'rb') as f:
    diag_proc_adj = pickle.load(f)

  with open('../drugrec/proc_diag_adj.pkl', 'rb') as f:
    proc_diag_adj = pickle.load(f)

  with open('../drugrec/prescriptions_adj.pkl', 'rb') as f:
    prescriptions_atc3_adj = pickle.load(f)

  with open('../drugrec/ddi_A_final.pkl', 'rb') as f:
    ddi_atc3_adj = pickle.load(f)

  voc = dill.load(open('../drugrec/voc_final.pkl', 'rb'))
  diag_voc, pro_voc, med_voc = voc['diag_voc'], voc['sym_voc'], voc['med_voc']

  num_diag_nodes = len(diag_voc["idx2word"])
  num_proc_nodes = len(pro_voc["idx2word"])
  num_clinical_nodes = num_diag_nodes + num_proc_nodes
  num_med_nodes = len(med_voc["idx2word"])
  ...
  return patient_info, ehr_KGs, ddi_KG, ddi_atc3_adj, voc,num_clinical_nodes,num_med_nodes