...
class GCN(nn.Module):
  ...
  def forward(self, x, edge_index, edge_weight):
    ...
    x = self.gnn1(x.to(device), edge_index.to(device))
    x = self.relu(x)
    x = self.dropout(x)
...
class KGDNet(nn.Module):
  def __init__(
    ...
    self.medicine_gnn = GCN(self.num_medicine_nodes, embed_dim, embed_dim, embed_dim)
    self.ddi_gnn = GCN(self.num_medicine_nodes-1, embed_dim, embed_dim, embed_dim)
  ...
  def forward(self, medical_data, ddi_KG):
    ...
    for i in range(num_adm):
      ...
      print("clinical_data 结构检查:")
      print(clinical_data)  # 检查节点数、边索引、edge_type是否合法
      print("clinical_data.x shape:", clinical_data.x.shape)
      print("clinical_data.edge_index shape:", clinical_data.edge_index.shape)
      print("clinical_data.edge_type shape:", getattr(clinical_data, 'edge_type', None))
      print(type(clinical_loader))
      print("Model device:", next(self.clinical_gnn.parameters()).device)
      
      clinical_emb = torch.zeros(self.num_clinical_nodes, self.embed_dim, self.embed_dim).to(device)
      print("ok")
      for batch_data in clinical_loader:
        print("Batch device:", batch_data.x.device)
        print(batch_data) 
