...
class DMNC(nn.Module):
    def __init__(self, vocab_size, emb_dim=64, device=torch.device('cpu:0')):
        ...
        K = len(vocab_size) #3,两个输入视图 + 一个目标输出视图
        self.K = K
        self.vocab_size = vocab_size
        self.device = device
    ...
    def forward(self, input, i1_state=None, i2_state=None, h_n=None, max_len=20):
        # 在每次迭代中，模型将当前 token 的嵌入向量 item_tensor 与从两个 DNC 编码器读取的记忆向量 r1, r2 拼接起来，作为解码器的输入。
        # 解码器生成新的隐藏状态 h_n 和输出 o3。
        # 使用 decode_read_variable 方法生成读取键、强度和模式，并通过 read_from_memory 方法从两个 DNC 编码器的记忆模块中读取新的记忆向量 r1, r2。
        # 将读取的记忆向量与解码器的输出结合，经过线性变换后得到当前时间步的输出 logits。
        # 应用 Softmax 函数将 logits 转换为概率分布，并选择具有最高概率的药品编号作为下一个生成的 token。
        # 如果生成的 token 是结束标记 <end>，则停止生成；否则，继续生成下一个 token。
        # input (3, code)
...
class Retain(nn.Module):
    ...
    def forward(self, input):
        ...
        # 修改部分：处理单次visit情况
        if len(input) == 1:
            # 对于单次visit，直接使用当前visit的embedding
            g = visit_emb.unsqueeze(0)  # (1, emb)
            h = visit_emb.unsqueeze(0)  # (1, emb)
        else:
            # 多次visit的原始处理逻辑
