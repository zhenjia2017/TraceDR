...
class Model(nn.Module):
    def __init__(self, n_sym, n_drug, ddi_adj, sym_sets, drug_multihots, embed_dim=64, dropout=0.4,
                 sym_embedding_path='symptom_embeddings.txt',
                 drug_embedding_path='drug_embeddings.txt'):
        ...
        self.load_embeddings(sym_embedding_path, drug_embedding_path)

        # Adjust DDI adjacency matrix to match n_drug
    ...
    def load_embeddings(self, sym_embedding_path, drug_embedding_path):
        """
        Load pre-generated symptom and drug embeddings from text files.
        """
        print("正在加载预生成嵌入...")

        # Load symptom embeddings
        sym_embeddings = np.loadtxt(sym_embedding_path, dtype=np.float32)
        if sym_embeddings.shape[0] != self.n_sym or sym_embeddings.shape[1] != self.embed_dim:
            raise ValueError(
                f"症状嵌入维度错误，预期 ({self.n_sym}, {self.embed_dim})，实际 {sym_embeddings.shape}"
            )
        # Initialize embedding matrix (0-th index unused, 1-based indexing)
        sym_embedding_tensor = torch.zeros(self.n_sym + 1, self.embed_dim, dtype=torch.float32)
        sym_embedding_tensor[1:] = torch.tensor(sym_embeddings)
        self.sym_embeddings.weight.data.copy_(sym_embedding_tensor)
        self.sym_embeddings.weight.requires_grad = False  # Freeze embeddings
        print(f"症状嵌入已加载从 {sym_embedding_path}")

        # Load drug embeddings
        drug_embeddings = np.loadtxt(drug_embedding_path, dtype=np.float32)
        if drug_embeddings.shape[0] != self.n_drug or drug_embeddings.shape[1] != self.embed_dim:
            raise ValueError(
                f"药物嵌入维度错误，预期 ({self.n_drug}, {self.embed_dim})，实际 {drug_embeddings.shape}"
            )
        # Initialize embedding matrix (0-th index unused, 1-based indexing)
        drug_embedding_tensor = torch.zeros(self.n_drug + 1, self.embed_dim, dtype=torch.float32)
        drug_embedding_tensor[1:] = torch.tensor(drug_embeddings)
        self.drug_embeddings.weight.data.copy_(drug_embedding_tensor)
        self.drug_embeddings.weight.requires_grad = False  # Freeze embeddings
        print(f"药物嵌入已加载从 {drug_embedding_path}")
        print("嵌入加载完成")
    ...
    def sparse_bmm_chunked(self, sparse_mat, adj, chunk_size=50000):
        n_drug = adj.shape[1]
        ...
        for i in range(0, n_drug, chunk_size):
            ...
            sub_adj = torch.sparse_coo_tensor(sub_indices, sub_values, (n_drug, end - i),
                                              device=sparse_mat.device).coalesce()
    ...
    def evaluate(self, syms, device='cpu'):
        ...
        drug_embeds = self.drug_embeddings(torch.arange(1, self.n_drug + 1).long().to(device))
